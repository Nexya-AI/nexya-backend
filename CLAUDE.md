...
|---|---|---|
| LlmProvider ABC | `ai/providers/base.py` | ❌ |
| OpenAI Provider (GPT-4o, DALL-E, Whisper, TTS) | `ai/providers/openai_provider.py` | ❌ |
| Gemini Provider (Vision + vidéo) | `ai/providers/gemini_provider.py` | ❌ |
| Qwen Provider (experts + NYLI) | `ai/providers/qwen_provider.py` | ❌ |
| LlmRouter (sélection + fallback) | `ai/router.py` | ❌ |
| QueryEngine (cycle de vie d'un turn) | `ai/engine/query_engine.py` | ❌ |
| StreamHandler (SSE + annulation Redis) | `ai/engine/stream_handler.py` | ❌ |
| SessionStore (Redis TTL 24h + flush PostgreSQL) | `ai/engine/session_store.py` | ❌ |
| ContextBuilder (prompts système experts) | `ai/context_builder.py` | ❌ |
| CostTracker (tokens + quotas + usage_daily) | `ai/cost_tracker.py` | ❌ |

### Feature Chat
| Endpoint | Fichier | Statut |
|---|---|---|
| `POST /chat` (réponse complète) | `features/chat/router.py` | ❌ |
| `POST /chat/stream` (SSE) | `features/chat/router.py` | ❌ |
| `POST /chat/stop` (annulation) | `features/chat/router.py` | ❌ |
