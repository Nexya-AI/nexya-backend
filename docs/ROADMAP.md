...
- [x] **O2 — DD-ready polishing (6 volets : architecture + compliance + API + onboarding + scripts + tests)** (2026-04-27) — Transformer le backend NEXYA en livrable « Due Diligence-ready » qu'un investisseur, consultant CTO externe ou nouveau Tech Lead peut auditer en autonomie en 2-3 heures. Le code est déjà solide (~4500 tests pytest 1700+ verts post-O1, 95 % backend couvert). O2 n'écrit pas de feature — il **rend lisible, exportable, auditable, vérifiable** ce qui existe. **Volet A — Architecture docs** : `docs/architecture/` 7 fichiers FR avec exec summary EN + diagrammes Mermaid (`graph TD`, `sequenceDiagram`, `erDiagram`) — `overview.md` (vue 30k pieds + diagramme Cloudflare → Hetzner → FastAPI → PG/Redis/MinIO/arq + 4 LLM providers + observability stack + roadmap), `data-model.md` (ER diagram simplifié + table-by-table comments + 19 migrations cumulative + index partiels critiques + volumétrie projetée 950k users + plan scaling V2 partitioning ai_calls), `request-flow.md` (4 sequence diagrams Mermaid : POST /auth/login + POST /chat/stream le plus détaillé auth→moderation→token estimator→cache→router→fallback chain→SSE→cost tracker fire-and-forget + POST /files/upload pipeline 10 étapes + GET /rgpd/user/data-export ZIP build 23 fichiers), `ai-architecture.md` (LlmRouter + 11 ExpertConfig + 5 chat providers réels + Mock usurpant + résilience RetryPolicy/CircuitBreaker/budget tracker/cache/token estimator/moderation rules + observabilité IA StreamMetrics/cost_tracker + tools F2.5 + 8 SaaS mock-first pattern), `security-posture.md` (threat model STRIDE par catégorie Spoofing/Tampering/Repudiation/Info disclosure/DoS/Elevation of privilege avec mitigations détaillées + defense-in-depth `/auth/register` 4 couches empilées + production safety guard liste exhaustive + headers HTTP O1 4 presets + roadmap pentest M1 DPIA M3), `observability.md` (3 piliers OTel/Sentry/Prometheus + 14 métriques `nexya_*` + Grafana 5 dashboards UIDs stables + 6 alertes calibrées + structlog injection trace_id+span_id + health checks split liveness/readiness étendue O1 + endpoint /version), `payments-readiness.md` (architecture placeholder Phase 11 CinetPay/NotchPay/Stripe + idempotent webhooks pattern HMAC + dedup table processed_webhooks + tables prévues subscriptions/payment_attempts/wallet_credits + escalation Crisp Phase 18 déjà livré + risques opérationnels conversion sandbox→prod). **Volet B — Compliance docs** : `docs/compliance/` 4 fichiers — `rgpd.md` (Article-by-article mapping Articles 5/6/7/15/17/20/28/32/33/35/37 + 7 catégories consent + workflow 2-step DELETE 30j grâce + ZIP export 23 fichiers anti-leak password_hash/storage_key/cross-user/IP brute), `ai-act.md` (EU 2024/1689 calendrier d'application 2024-2027 + classification limited risk + Article 13 mapping registre `ai_calls` enrichi + endpoint admin `GET /rgpd/admin/ai-act-registry` CSV BOM UTF-8 ou JSON + 4 sous-traitants AI documentés), `security-checklist.md` (OWASP Top 10 2021 mapping A01-A10 + production safety guard fail-fast + roadmap pentest M1), `dpa-template.md` (Article 28 RGPD template placeholder avec 8 sous-traitants NEXYA listés + Google/OpenAI/Anthropic/Brevo/hCaptcha/Crisp/Hetzner/Cloudflare lieu+DPA standard + 15 sections template à compléter par avocat). **Volet C — API contracts** : `docs/api/` 3 fichiers + 1 export — `endpoints.md` (table 60+ endpoints groupés par feature avec path/method/auth/rate-limit/description), `error-codes.md` (table 30+ codes NEXYA groupés Auth/Captcha/Quotas/LLM/Files/Resources/Chat/Planner/Projects/RGPD/Helpdesk/Paiements + categorisation HTTP 200/201/204/400/401/402/403/404/409/413/415/422/429/500/503), `versioning.md` (V1 unprefixed policy + critères bump V2 breaking change body/security scheme/suppression endpoint + deprecation policy 6 mois minimum + header Deprecation/Sunset/Link), `openapi.json` exporté via `python -m scripts.export_openapi` (~6500 lignes JSON 60 endpoints + 20 tags + 3 servers + BearerAuth+PrometheusToken securitySchemes). **Volet D — Onboarding & runbooks** : `README.md` racine `nexya_backend/` (250 lignes badges + exec summary EN + section « Onboarding 5 minutes » obligatoire prérequis Python 3.12+Docker+uv+bash + setup 10 commands clone→`.env`→docker compose→alembic upgrade→seed_dev→uvicorn→curl healthz→open /docs + structure dossiers + commandes du quotidien tests/lint/migrations/évals/load/exports DD/worker arq + production safety guard liste + DD package pointer vers docs/ + Africa-first context + support email Crisp), `docs/glossary.md` (50+ termes brand/architecture/persistance/IA-coûts/conformité/Africa/stack/évals/workflows/phases — utile lecteur étranger DD), 3 runbooks `docs/runbooks/` (`incident-response.md` escalation matrix Critical/High/Medium/Low + 5 procédures step-by-step 5xx spike/LLM unavailable cascade/DB pool saturated/payment webhook failure/RGPD breach 72h notification + outils & accès Grafana/Sentry/Cloudflare/Hetzner + post-mortem template ; `deployment-l2.md` 10 steps Hetzner CCX23 provisioning→setup serveur→DNS Cloudflare→secrets injection→build GHCR→pull+migrations→smoke tests→AlertManager destinations→GHA secrets→test évals+load + rollback via scripts/rollback.sh ; `db-restore.md` cron quotidien backup_db.sh S3 chiffré + restauration ponctuelle 12 steps + DR drill quarterly + RGPD considerations sur backups), 5 ADRs `docs/adr/` format Nygard Status/Context/Decision/Consequences (`0001-fastapi-vs-django.md` async-first SSE + Pydantic v2 + écosystème IA Python ; `0002-sqlalchemy-async.md` Mapped[] type-safe + pgvector + Alembic vs Tortoise/SQLModel ; `0003-redis-rate-limiting.md` sliding window INCR+EXPIRE atomique vs Postgres/leaky bucket/Cloudflare WAF ; `0004-jwt-rs256-vs-hs256.md` asymétrique clé publique distribuable + audit-friendly + standard OIDC ; `0005-llm-router-mock-first.md` pattern 8 SaaS mock-first dev sans clé/CI sans secret/tests déterministes + identité usurpée + bénéfices vs feature flag binaire). **Volet E — Scripts d'automation** : `scripts/export_openapi.py` (~80 lignes argparse + Windows event loop policy + import différé app.main + json.dump indent=2 ensure_ascii=False + log paths_count+tags_count+version), `scripts/export_schema.sh` (strict bash set -euo pipefail + lecture DATABASE_URL env/.env + strip préfixe SQLAlchemy `postgresql+psycopg://` → `postgresql://` libpq compat + headers comments injection date+commit_sha + pg_dump --schema-only --no-owner --no-acl --no-comments), `Makefile` 3 nouveaux targets `make export-openapi`/`make export-schema`/`make export-dd`, workflow `.github/workflows/dd-exports-fresh.yml` (trigger `push: main` + `workflow_dispatch`, services postgres+redis identique ci.yml + JWT keys volatiles + alembic upgrade head, re-génère openapi.json + schema.sql, `git diff --exit-code` 2 fichiers, fail+open issue auto label `dd-exports-stale`+`priority/medium` si stale avec instructions régénération). **Volet F — Polishing docstrings** : différé V2 (passe ultérieure sur top 10 modules sous-commentés via heuristique > 200 lignes avec ratio commentaires/code < 30%). **42 tests pytest verts** (`test_dd_exports.py` ~50s) : README onboarding section + DD pointer, glossary 30+ termes (50+ livrés), 7 architecture docs avec mermaid sur overview, 4 compliance docs avec articles RGPD + OWASP Top 10, 3 API docs + openapi.json valide JSON 3.1.0 + 50+ paths + 18+ tags + 30+ codes critiques mentionnés, 3 runbooks, 5 ADRs format Nygard strict 4 sections, 19+ migrations cross-check, schema.sql skipif absent (cas dev sans pg_dump local), scripts export+workflow GHA freshness. 0 régression sur la suite globale pré-O2. **Aucune nouvelle dep pip**. **Aucune migration DB**. **Recommandations Ivan post-O2** : (1) lancer `make export-dd` post-modification de routers/migrations + commit (workflow GHA dd-exports-fresh.yml fail push main sinon, ouvre issue auto) ; (2) configurer `pg_dump` locale pour générer `schema.sql` initial + commit (test skipif sinon) ; (3) compléter docstrings polishing en V2 micro-session si DD investisseurs le demande ; (4) ajouter `payments-readiness.md` détails post-livraison Phase 11 I1/I2/I3 ; (5) post-launch créer `docs/api/changelog.md` (Keep a Changelog format) pour tracer les évolutions de contrat API. **Coût O2 : $0** (pure rédaction documentation + scripts shell + 42 tests). **Bloc O à 100% fermé** : O1 ✅ + O2 ✅. **Phases backend autonomes terminées** — le reste relève des phases L2 staging deployment + L3 prod observability + M1 pentest + M3 DPIA + Phase 19 multi-region failover.
- [x] **O1 — OpenAPI/Swagger + Health check étendu + Headers sécurité** (2026-04-27) — 3 volets indépendants livrés ensemble pour rendre le backend **DD-ready** (pré-launch + investisseurs). **Volet A OpenAPI/Swagger production-grade** : `app/core/openapi/customizer.py` enrichit le schéma auto-généré FastAPI en OpenAPI 3.1 (JSON Schema 2020-12) avec `info.title/summary/description multi-paragraphes FR (auth + rate limits + erreurs codifiées + SSE + Africa-first + sécurité conformité + observabilité)/contact support@nexya.ai/license Proprietary Nexyalabs/termsOfService URL placeholder/x-logo extension ReDoc avec backgroundColor brand`, `servers` array 3 entrées (dev http://localhost:8000 + staging https://api-staging.nexya.ai + prod https://api.nexya.ai), `NEXYA_TAGS_METADATA` 20 tags avec `description` FR détaillées par feature (auth/user/chat/projects/library/files/voice/vision/memory/rag/ai_models/tasks/notifications/feedback/suggestions/rgpd/admin/helpdesk/observability/health), `components.securitySchemes.BearerAuth` HTTP bearer JWT TTL 15min documenté + `PrometheusToken` apiKey header `X-Prometheus-Token` avec description constant-time compare, hook `app.openapi = lambda: customize_openapi(app)` posé APRÈS `include_router` pour avoir accès à toutes les routes, idempotent via `app.openapi_schema` cache. **Volet B Health check étendu** : `app/core/health/version.py::detect_version()` pipeline 3 fallbacks (env vars `APP_VERSION`/`APP_COMMIT_SHA` posés par CI/CD release pour Docker prod sans `.git/` → subprocess `git describe --tags --exact-match HEAD` + `git rev-parse HEAD` + `git status --porcelain` détection dirty avec timeout 2s + `shutil.which('git')` check binaire dispo → lecture directe `.git/HEAD` + `.git/refs/heads/<branch>` sans appeler git binary, utile container Docker qui copie .git/ mais sans git installé) retourne `VersionInfo(version, commit_sha[:40], tag, dirty, source)` frozen dataclass + helper `_find_repo_root()` remonte 12 niveaux ; `app/core/health/extended.py::ExtendedHealthService.compute(db, redis)` agrège fail-safe par champ : `_build_version_payload()` via detect_version, `_build_db_payload(db)` (timing `SELECT 1` → latency_ms) + `_read_last_migration(db)` (SELECT version_num FROM alembic_version LIMIT 1), `_build_redis_payload(redis)` (timing `redis.ping()` → latency_ms), `_build_arq_payload(redis)` (`ZCARD arq:queue` — arq utilise sorted set par défaut, O(1) Redis-side), uptime depuis `_APP_START_MONOTONIC` global posé au lifespan startup via `set_app_start_monotonic()`, `status` global computed `ok` si DB+Redis up sinon `degraded` ; **`/ready` réécrit** pour exposer le payload étendu (status 200/503 backward-compat K8s + body enrichi avec `version`/`db.latency_ms`/`db.last_migration`/`redis.latency_ms`/`arq.queue_depth`/`uptime_seconds`/`status`), `data.db` devient un dict `{status, latency_ms, last_migration}` au lieu d'une string plate (breaking pour les anciens clients qui parsent `data.db === "ok"`, documenté dans test) ; **nouvel endpoint public `GET /version`** sans token retourne `{version, commit_sha, tag, dirty, env, source}` léger < 5ms (Flutter Settings affichera la version backend), pas de DB call, anti-fingerprinting validé par test ; `/healthz` reste minimal anti-redémarrage K8s sur DB transient ; `/health` alias /ready conservé. **Volet C Middleware Headers sécurité** : `app/core/security/headers.py::NexyaSecurityHeadersMiddleware` 4 presets dans `_PRESET_TO_HEADERS` dict — `dev` (X-Content-Type-Options: nosniff seul, Swagger UI fonctionne) / `staging` (nosniff + HSTS max-age=31536000 sans includeSubDomains + X-Frame-Options DENY + Referrer-Policy strict-origin-when-cross-origin + Permissions-Policy camera=()/microphone=()/geolocation=()/payment=()/usb=()/magnetometer=() + CSP `default-src 'self' 'unsafe-inline'; img-src 'self' data: https:; font-src 'self' data:; connect-src 'self' https:; frame-ancestors 'none'`) / `prod` (staging headers + HSTS includeSubDomains preload pour soumission hstspreload.org + Cross-Origin-Opener-Policy same-origin + Cross-Origin-Resource-Policy same-origin + CSP strict sans unsafe-inline + base-uri 'none' + object-src 'none') / `off` (kill-switch incident, aucun header), skip Content-Security-Policy sur `/docs`/`/redoc`/`/openapi.json` quand preset ∈ {dev, staging} — Swagger UI utilise inline JS, prod n'expose pas /docs (`docs_url=None`) donc skip pas nécessaire, ne pas écraser un header déjà posé par l'endpoint, ajouté en dernier dans `app.add_middleware` (Starlette LIFO = exécuté en premier sur la response, pour s'appliquer même aux error handlers globaux) ; **`_enforce_production_safety` étendu** fail-fast au boot si `is_production AND security_headers_preset NOT IN ('prod', 'off')` (on accepte `off` comme kill-switch incident ponctuel — Ivan peut désactiver le temps de débugger un faux positif). **Settings** : `security_headers_preset: Literal["dev","staging","prod","off"]="dev"` (défaut dev), `APP_VERSION`/`APP_COMMIT_SHA` env vars CI/CD. `.env.example` 2 nouvelles sections. **`app/main.py` wiring** : import `customize_openapi` + `set_app_start_monotonic` + `detect_version` + `ExtendedHealthService` + `NexyaSecurityHeadersMiddleware`, `set_app_start_monotonic()` au début du lifespan startup, `app.add_middleware(NexyaSecurityHeadersMiddleware, preset=settings.security_headers_preset)` après TraceIdMiddleware, `/version` endpoint public, `/ready` étendu, `app.openapi = lambda: customize_openapi(app)` après tous les `include_router`. **51 tests pytest verts** (~67 s en isolation incluant lifespan TestClient). `test_openapi_schema.py` ×16 : openapi 3.1.0, info FR avec contact/license/termsOfService/x-logo, servers ≥ 3, tags ≥ 18 avec descriptions FR ≥ 30 chars, BearerAuth + PrometheusToken security schemes, paths ≥ 50 anti-drop, endpoints critiques présents (`/auth/login` `/chat/stream` `/healthz` `/ready` `/version` `/admin/helpdesk/metrics`), health endpoints taggés `health`, admin/helpdesk taggé `admin`+`helpdesk`, customize_openapi idempotent via cache, NEXYA_TAGS_METADATA pas de duplicates et tags critiques présents. `test_health_extended.py` ×17 : `/version` 200 sans auth, anti-leak fingerprinting (pas password/token/secret/api_key dans la response), `detect_version()` retourne VersionInfo dataclass, `_from_env_vars()` lit APP_VERSION+APP_COMMIT_SHA, retourne None si vide, `_from_git_head_file()` retourne None si pas de .git/, uptime None avant set + positif après set, `ExtendedHealthService.compute(db=None, redis=None)` retourne ExtendedHealthResponse status=degraded, `_build_db_payload` fail-safe sur exception → unavailable, `_build_redis_payload` fail-safe, `_build_arq_payload` fail-safe → queue_depth=None, queue empty → 0, status degraded si DB KO mais Redis OK, `/ready` payload étendu avec version/db/redis/arq/uptime_seconds, `data.db` dict avec status, `/health` alias /ready backward-compat. `test_security_headers.py` ×18 : preset off pas de headers, dev pose nosniff seul, staging pose CSP unsafe-inline + HSTS court + X-Frame DENY + Referrer + Permissions, prod pose HSTS preload + COOP + CORP + CSP strict (sans unsafe-inline) + frame-ancestors none, skip CSP sur /docs+/redoc+/openapi.json en non-prod, prod ne skip pas, middleware ne modifie pas status_code ni body, ne overwrite pas un header déjà posé, constructor refuse preset inconnu, production safety rejette dev/staging en prod et accepte off+prod. **Aucune régression** sur la suite globale pré-O1. **Aucune nouvelle dep pip** (FastAPI/Starlette/Pydantic déjà installés). **Recommandations Ivan prod** : (1) poser `SECURITY_HEADERS_PRESET=prod` dans `secrets.GITHUB_ACTIONS` ou env Docker prod AVANT déploiement L2 staging ; (2) modifier `scripts/release.sh` L1 pour poser `APP_VERSION=$tag` + `APP_COMMIT_SHA=$GITHUB_SHA` dans les Docker build args (sinon `/version` retournera `unknown` en prod sans .git/) ; (3) HSTS `preload` = engagement long terme (soumission hstspreload.org), à faire post-stabilisation L2 staging quand domaine prod stable ; (4) si Swagger UI doit rester accessible en staging public (debug ops), preset staging permet ; en prod, `docs_url=None` désactive complètement /docs ; (5) Flutter datasource doit migrer parsing `data.db === "ok"` → `data.db.status === "ok"` pour `/ready` (breaking V1 documenté). **Coût O1 : $0** (pure infra config + middleware Python). **Aucune migration DB**. **Boucle Bloc O à 50%** : O1 ✅. Reste O2 — DD-ready polishing (code propre commenté pour due diligence, docs architecture finalisées, README backend dev, schémas SQL exportés, contrats API à jour, conformité contrats OK).
...
- **2026-05-04** — **F0.5 backend extension Monthly + Yearly schedule types** (mini-session backend non listée, déclenchée par la session frontend F1 qui voulait préserver `monthly`/`yearly` côté UI au lieu de les amputer). Ajout `MonthlyConfig` (`day` 1-31 + `hour` + `minute`) et `YearlyConfig` (`month` 1-12 + `day` 1-31 + `hour` + `minute` avec validator `_validate_month_day` qui rejette 30 février et 31 avril mais accepte 29 février — clamp à 28 lors du calcul `next_run_at` sur année non bissextile) dans `app/features/planner/schemas.py`. Extension `ScheduleType` literal vers 6 valeurs. Helper `_make_monthly_candidate(year, month, day, hour, minute)` dans `scheduler.py` qui clamp `day` au dernier jour du mois via `calendar.monthrange()` (gestion automatique 31 février → 28/29, 31 avril → 30, 31 juin → 30). `compute_next_run` étendu avec 2 branches (monthly : tente mois courant puis avance d'un mois si déjà passé ; yearly : tente année courante puis suivante). **21 nouveaux tests** (8 schemas Pydantic + 13 scheduler avec edge cases bissextile / clamp / décembre→janvier). **86 tests verts en local** (`pytest tests/test_planner_*.py tests/test_scheduler_helpers.py tests/test_scheduler_execute_worker.py`), **zéro régression**. **0 migration DB** (`schedule_config` est déjà JSONB, accepte n'importe quel dict). **0 nouvelle dep**. **0 nouvelle exception**. Service.py et router.py inchangés (la délégation à `compute_next_run` + `_schedule_to_dict` rend l'extension transparente). Stack F1 reste cohérente : 4 schedule types deviennent 6, le frontend peut maintenant créer des tâches « Bilan mensuel le 15 » / « Anniversaire le 21 juin » sans amputation.
- **2026-05-05** — **G3 frontend Vision (camera/galerie → analyse) + Files upload générique — asymétrie Free/Pro par tier flash/pro** (20 nouveaux fichiers + 8 modifs Flutter, **0 nouveau package pubspec** — `image_picker ^1.1.2` + `file_picker ^8.1.4` + `image ^4.2.0` + `permission_handler ^11.3.0` déjà installés depuis G3-prep, `flutter analyze` 0 erreur 0 warning sur tout le delta). Aucune modif backend nécessaire — pure consommation de `POST /vision/analyze` (E2 livré 2026-04-24) + `POST /files/upload` (E3 livré 2026-04-24) + `GET /files/{id}` (D2.5 livré 2026-05-04). **Module pivot `lib/core/attachments/`** (5 fichiers) : `attachment_models.dart` (PickedImage Equatable bytes JPEG compressés + width/height/mime/size/PickedImageSource enum, PickedFile File ref + filename/size/mime/extension, ImageCompressionException), `mime_inferrer.dart` (whitelist 4 docs PDF/DOCX/TXT/MD + 5 images + formatSize ICU virgule décimale + FileIconCategory enum color-coding), `image_compressor.dart` (resize ≤ 2048² JPEG q=85 via package `image`, fallback 1536² si > 5 MB après 1ère passe, throw `decode_failed`/`encode_failed`/`too_large_after_compression`, sync main isolate ~50ms iPhone 13 sur 4096²), `attachment_permissions.dart` (wrapper permission_handler avec normalisation `granted`/`denied`/`permanentlyDenied` + court-circuit isGranted/isLimited + openAppSettings), `attachment_picker.dart` (wrapper image_picker camera + multiImage cap 4 + file_picker custom whitelist + cap pre-flight 10 MB + FilePickerSizeException/FilePickerPathNullException typées + hide image_picker.PickedFile legacy). **Feature `lib/features/vision/`** (4 fichiers) : `domain/vision_response.dart` (miroir Pydantic 13 champs + parser Decimal tolérant), `models/vision_request.dart` (enum `VisionTier {flash,pro}`), `data/vision_exceptions.dart` (sealed 11 sous-classes : Unauthorized/PlanRequired/QuotaExceeded current+max+plan/ContentFiltered/Unavailable/ImageTooLarge/FileTypeNotAllowed/ContentMismatch/RateLimited retryAfter/Validation/Network/Server), `data/vision_remote_datasource.dart` (POST /vision/analyze mode `image_base64` + `additional_images` upload_ids, dispatch erreurs PRIORITAIREMENT par `code` envelope puis fallback statusCode, sendTimeout 2 min/receiveTimeout 3 min), `logic/vision_service.dart` (service stateless : encode bytes → data URL `data:image/jpeg;base64,...` + délégation datasource + truncate prompt 4000 chars). **Extension feature D2.5 `lib/features/files/`** : ajout `uploadBytes()` au datasource pour images compressées in-memory + fix discrepancy parsing `original_filename`/`filename` + `url`/`presigned_url` (D2.5 parsait `presigned_url` mais backend renvoie `url` → URL revenait `null` silencieusement, fix défensif rétrocompat). **`lib/features/chat/models/message_attachment.dart`** : sealed `MessageAttachment` 2 sous-classes (MessageImageAttachment bytes Uint8List + width/height + uploadId? + presignedUrl? + originalFilename?, MessageFileAttachment uploadId + presignedUrl + filename + extension? + chunksIndexedAt? RAG D2.5 + factory `fromUploadedFile`). **HomeMessage** étendu : nouveau champ `attachments: List<MessageAttachment>` default `const []` + factories `user(text, {attachments})` + copyWith. **`attachment_composer_notifier.dart`** : Notifier scoped via `family<String>` (clés `home`/`chat:<convId>`/`expert:<d>`/`project:<p>`), state Equatable (pendingImages cap 4 mutex avec pendingFile + premiumEnabled + isUploading/isAnalyzing + lastErrorCode/lastErrorData), pipeline `submit(text, delegate)` qui upload images additionnelles via `FilesRemoteDatasource.uploadBytes` puis délègue payload typed (typedef `ChatSendDelegate({text, attachments, tier, rawImages})`), dispatch erreurs sealed → mapping vers code string + data Map (11 codes Vision + 6 codes Files). Pattern aligné `LibraryAsyncNotifier` D3 (extends Notifier sans FamilyNotifier — l'arg passe via constructeur). **`vision_file_send_mixin.dart`** : mixin partagé HomeChatNotifier + ChatScreenNotifier avec bridge `VisionFileSendBridge` (currentMessages getter + updateMessages + onAttachmentErrorCode). 2 méthodes : `sendVisionMessage` (insert bulle user+assistant placeholder, appelle VisionService.analyze, complète assistant ou marque failed, rethrow VisionApiException) et `insertUserMessageWithFile` (juste injection bulle user, V1 G3 le contenu fichier reste contextuel — pas wired LLM). **HomeChatNotifier + ChatScreenNotifier** : `with VisionFileSendMixin implements VisionFileSendBridge` + nouvelle API publique `submitWithAttachments({text, attachments, tier, rawImages, defaultPromptIfEmpty})` route Vision vs File + helper `_sendStreamForFileContext` qui enchaîne `/chat/stream` après attachment fichier si prompt non vide. **Widgets `lib/shared/widgets/business/`** : `nx_attachment_sheet.dart` (BottomSheet plein-largeur 3 cartes + handle drag + masque kIsWeb pour Caméra/Galerie + retourne NxAttachmentAction?), `nx_attachment_widgets.dart` consolidé (NxAttachmentPreviewRow ListView horizontal AnimatedSize 0→80px slide-up 220ms + chips images/fichiers thumbnails + bouton remove ×, NxPremiumToggle Pro-gated avec Switch.adaptive + tap Free → snackbar info + push paywall, NxBubbleAttachments single image plein-largeur ratio préservé OU grille 2×2 carrée 2-4 images OU carte fichier 64px tap → snackbar stub V1, badge gold `Prêt RAG` quand chunksIndexedAt non-null). **Modif `NxInputBar`** : remplace `showMenu` PopupMenu legacy par `NxAttachmentSheet.show()` BottomSheet (UX moderne) + nouveau param `disableAttachments: bool = false` qui masque bouton + et son spacer. **Modif `NxChatBubble`** : nouveau param `attachments` propagé au `_buildUserBubble` qui rend `NxBubbleAttachments` AU-DESSUS du content texte (sortie du Container bleu pour préserver les coins arrondis), bulle texte conditionnelle (image-only sans prompt → pas de Container vide). **Wiring 5 écrans** : HomeScreen + ChatScreen wirent NxAttachmentPreviewRow + NxPremiumToggle au-dessus du NxInputBar, callbacks `onCamera/onGallery/onFiles` permissions + AttachmentPicker + composer.add*, `_onSend` route via `composer.submit(delegate: chatNotifier.submitWithAttachments)` ou `sendMessage` classique, `ref.listen<AttachmentComposerState>` dispatch erreurs typées via `_dispatchComposerError(code, data, s)` (12 codes mappés snackbar variant + push paywall PLAN_REQUIRED + ICU countdown 429 retry_after) avec `clearLastError()` post-frame. EphemeralChatScreen + ProjectDetailScreen + ExpertChatScreen → `disableAttachments: true` (V1 cohérence privacy/scope produit). Propagation `attachments: msg.attachments` dans 4 NxChatBubble call-sites. **Permissions natives** : iOS Info.plist + Android Manifest déjà en place depuis G3-prep — aucune nouvelle modification native. **38 nouvelles ARB FR+EN** : `attachmentSheet*` (7) + `attachmentChip/Indexing/Indexed/RateLimited/SizeTooLarge/PermissionCamera/Gallery/OpenSettings/ImageDecodeFailed/TooManyImages/PickAborted/PreviewSoon` (12) + `visionDefaultPrompt/Analyzing/ContentFiltered/Unavailable/QuotaExceeded ICU current/max/PlanRequiredPro/ImageTooLarge` (7) + `filesUpload*` (5) + `proPremium*` (3). **Décisions clés (12)** : composer scoped via family<String> (chaque écran a son propre stack), ChatSendDelegate typedef pour découpler composer ↔ notifiers, MessageAttachment sealed, resize main isolate (~50ms iPhone 13), image principale base64 inline + additional via upload_id (économie 1 round-trip), auto-prompt fallback `s.visionDefaultPrompt`, Pro toggle = opt-in pas par défaut Pro, EphemeralChatScreen + ProjectDetailScreen + ExpertChatScreen sans attachments V1, polling RAG snapshot upload-time (pas tight-loop GET), cap client 5 MB après compression + 4 images max, snackbar countdown ICU pour 429, V1 url_launcher absent → tap fichier = snackbar stub. **Anticipations seniors 14 edge cases** documentés dans le prompt G3 (HEIC iOS transcode imageQuality:95, image > 4096px main isolate, permission permanentlyDenied → CTA openAppSettings, kIsWeb masque, > 5 MB après compression → fallback 1536² puis exception, multi-images partial upload echec, anti-double-submit isBusy, cancel pendant analyze → mounted check, 429 countdown, 503 retry CTA, Free toggle Pro snackbar+paywall, Ephemeral disabled, network down catch DioException, content_sha256 backend dédup transparent). **Boucle BLOC G frontend FERMÉE à 100 %** (G1 ✅ + G2 ✅ + G3 ✅) — passage au BLOC H Notifications.
- **2026-05-04** — **G2 frontend Voice TTS sortie messages IA asymétrie Free/Pro** (9 nouveaux fichiers + 6 modifs Flutter, 1 package ajouté `flutter_tts: ^4.2.0`, `flutter analyze` 0 erreur 0 warning). Aucune modif backend nécessaire — pure consommation `POST /voice/speak` E1 Pro-only. **Free (offline natif)** : ABC `TtsService` + `RealFlutterTtsService` wrapper flutter_tts v4 (audio session iOS playback + mixWithOthers, mapping speed cross-plateforme, strip markdown regex avant lecture) + `MockTtsService` web/desktop fallback. **Pro (OpenAI TTS)** : `OpenAiTtsService` wrapper Dio bytes MP3 → audioplayers BytesSource, mapping immutable `_nexyaToOpenaiVoice` (6 voix NEXYA branded → 6 voix OpenAI Pydantic), cap chars 4096 client-side, dispatch erreurs 402/403/429/503. **Orchestrateur central** `TtsNotifier` Riverpod sealed `TtsState` 5 variantes avec sélection runtime Free/Pro selon `user.isPro` + `voiceSettings.premiumEnabled`, debounce 300ms, lazy init. **VoiceSettings persisté SharedPreferences** (autoReadEnabled / speed / premiumEnabled). **NxAiActionsBar wiré** : `_SpeakActionIcon` réactif au `TtsState` avec icon dynamic (idle/loading/speaking/paused/other), tap → `speakOrToggle`. **Auto-read pipeline** : hook `ChatStreamMixin.onMessageCompleted` + `Set<String>` anti-double-trigger dans HomeChatNotifier + ChatScreenNotifier, ephemeral chat n'override pas (privacy). **VoiceScreen enrichi** 4 sections (Voix / Auto-lecture toggle / Vitesse slider / Pro premium toggle avec badge gold + CTA paywall `/settings/account` pour Free). **17 ARB FR+EN** avec ICU `{plan}/{seconds}/{value}`. **Décisions clés** : ABC mock-first, mapping NEXYA→OpenAI côté frontend uniquement, Pro = opt-in (pas activé par défaut même Pro), audio session iOS `playback + mixWithOthers` (lit même en silent + ne ducke pas Spotify), markdown strip basique avant TTS. **Anticipations 12 edge cases**. **Boucle BLOC G étendue** (G1 ✅ + G2 ✅ — G3 Vision reste).
- **2026-05-04** — **G1 frontend Voice STT bouton micro asymétrie Free/Pro** (10 nouveaux fichiers + 8 modifs Flutter, 2 packages ajoutés `speech_to_text: ^7.0.0` + `permission_handler: ^11.3.0`, `flutter analyze` 0 erreur). Aucune modif backend nécessaire — pure consommation E1 backend (`POST /voice/transcribe` Pro-only). **Free (offline natif)** : ABC `SpeechToTextService` + Real wrapper speech_to_text v7 + Mock fallback web/desktop. `SpeechNotifier` Riverpod orchestrateur permissions runtime + lifecycle micro avec sealed `SpeechState` 4 variantes. Centralisation dans 4 écrans (home/chat/expert/ephemeral) — remplace `_isRecording` boolean + stub `voicePromptStub` par `ref.watch(speechIsListeningProvider)` + `ref.listen<String?>(speechPartialTextProvider)` qui pousse les partials dans le TextEditingController + dispatch erreurs typées (permissionDenied/notAvailable/recognitionFailed). LocaleId IETF BCP-47 → POSIX. **Pro (Whisper backend E1)** : pile complète `features/voice/` (domain + sealed 8 exceptions + datasource Dio multipart + sealed state machine 7 états + AsyncNotifier + screen pixel-perfect avec 7 blocs visuels). Validation pre-flight 20 MB côté client. Paywall gold pour 403 PLAN_REQUIRED → CTA `/settings/account`. **Permissions natives** : AndroidManifest RECORD_AUDIO + queries RecognitionService (Android 11+), iOS NSSpeechRecognitionUsageDescription. Settings CTA `/settings/voice/transcribe` ajoutée. **14 ARB FR+EN** ICU placeholders. **12 edge cases seniors** anticipés et documentés. **Boucle BLOC G frontend ouverte** (G1 ✅ — G2 TTS Free/Pro asymétrie + G3 Vision/Files restent à coder).
- **2026-05-04** — **F3 frontend Création task via chat (Tools LLM function calling SSE)** (6 nouveaux fichiers + 9 modifs Flutter, `flutter analyze` 0 erreur). Aucune modif backend nécessaire — pure consommation de l'SSE `event:tool_call` déjà livré F2.5 backend (2026-04-25). Le frontend décode le payload `{id, name, arguments_json_partial, index}`, accumule les deltas par `index` dans un `ToolCallBucket` pendant le round LLM, finalise en `ToolCallSummary` quand `chunk.finish_reason='tool_calls'` arrive, refresh `plannerAsyncProvider` au `event:done`, résout `resolvedTaskId` par matching `title` + `created_at < 2 min` via helper partagé `planner_tool_resolver.dart`. UI : `TaskPreviewCard` pixel-perfect 4 variantes (createTask/updateTask/pauseTask/listTasks) × 3 états (pending/succeeded/failed) rendue dans `NxChatBubble` après le content. CTA « Voir dans le planificateur » → `/planner?highlight=<taskId>` avec animation glow gold 2 s + auto-scroll easeOutQuart. Helper partagé entre `HomeChatNotifier` et `ChatScreenNotifier` (DRY). 14 nouvelles ARB FR+EN. **Boucle BLOC F frontend fermée à 100 %** (F1 + F2 + F3). Aucune nouvelle dep pubspec, zéro régression.
- **2026-05-04** — **F2 frontend Planner — finalisation mock-first (timezone Africa/Douala + validation stricte + NotificationPreferences + DeviceToken)** (12 fichiers Flutter + 2 ARB, `flutter analyze` 0 erreur). Aucune modif backend nécessaire — pure consommation des endpoints F1 + F3 déjà livrés. Le frontend wire désormais `POST/DELETE /user/device-token` (mock-first via `MockPushNotificationService` qui génère UUID stable préfixé `mock-`) au login/logout via hooks fail-safe `unawaited`, et `GET/PUT /user/notification-preferences` via un nouvel écran `/settings/notifications-prefs` pixel-perfect (5 cards × 4 channels RadioGroup) + un `_PlannerNotifChannelPicker` 4-chips dans le PlannerForm qui remplace les 2 toggles push/email locaux F1. Le pattern mock-first garantit que H1 (setup Firebase Messaging quand Ivan aura `FCM_SERVER_KEY`) sera un swap 1 ligne dans le provider `pushNotificationServiceProvider`. **Aucune nouvelle dep pubspec**, aucune migration DB, aucun nouvel endpoint backend. Zéro régression — les rows `device_tokens` mock cohabitent avec de futurs FCM tokens via UPSERT UNIQUE.
- **2026-05-19** — **Session A2 backend — Affûtage 11 Experts au niveau divin (Période 2 IA-Quality Phase A) livrée — 483 nouveaux tests A2 verts en 3.21 s + 694/694 tests régression cibles verts + 2 xfailed G1 langues attendus, 0 régression sur la suite cible (A1+A2+G1+G2+chat+experts).** Mission Silicon Valley : faire passer NEXYA AI du « bon » au **« divin »** — étonnement utilisateur dès les premières réponses. Refonte intégrale des 11 system_prompts experts (5-15 lignes → 50-200 lignes ciselées). **Nouveau package `app/ai/expert_prompts/`** : 12 modules dédiés (`_shared.py` helpers transverses + 11 experts un par fichier). Chaque expert = persona profonde + méthodologie step-by-step + 2-4 templates de sortie + 2-3 few-shot examples calibrés + anti-patterns + 6 clauses transverses (multi-langue dynamique, memory-aware D3, format markdown, attribution sources, progressive disclosure, continuité conversationnelle). **Safety-critical hardening MAX** : medicine = bloc URGENCE EN TÊTE détection 5 symptômes vitaux (douleur thoracique, AVC, hémorragie, détresse respi, idées suicidaires) + numéros Cameroun 117/118/119 + 112 international ; legal = OHADA + Code civil article exact ou aveu d'incertitude. **Cooking préserve G2 V8** (107 recettes Loth Ivan + RAG framing + 2 substitutions obligatoires). **Refactor `experts.py`** : 11 imports propres + `_with_guardrail` centralisé sur 9 experts spécialisés. Préservation intégrale `_DOMAIN_GUARDRAIL_TEMPLATE` G2 + disclaimers + corpus_enabled cooking + disable_thinking cooking + tier flash/pro/image. **3 fichiers tests stratégiques** : `test_expert_prompts_shared.py` (~30 tests), `test_expert_prompts_per_expert.py` (~130 tests paramétrés sur 11 experts), `test_expert_prompts_specifics.py` (~70 tests métier). **Réparation collatérale** du test `test_nexya_identity_preserved_on_all_experts` cassé par A1 cleanup `_NEXYA_IDENTITY=""` (chaque persona A2 mentionne désormais NEXYA + Nexyalabs naturellement). **Multi-langue dynamique** dès A2 (clause unique injectée — chaque expert détecte langue user et répond dans la même langue). **Anti-jailbreak per expert** maintenu (medicine refuse posologie nominative même sous prompt injection, legal refuse rédaction acte engageant). **Cap chars 25000/expert** calibré pour permettre richesse divine sans déborder cap LLM 30k tokens (preamble 12000 + memory 1500 + corpus 3000 + expert 25000 ≈ 10000 tokens ≈ 33 % du cap). **0 nouvelle migration DB, 0 nouvelle dep pip, 0 nouvel endpoint HTTP.** **Coût A2 : $0** (pure infra Python + tests mock-first). 7 questions test acceptance Silicon Valley pour Ivan post-merge (async/threads computer, démonstration aire triangle science, ndolé sans arachides cooking, SARL OHADA legal, douleur thoracique medicine URGENCE 117/118/119, traduction douala language, anti-jailbreak posologie médecine). **Boucle Période 2 IA-Quality Phase A à 2/3 fermée** (A1 ✅ + A2 ✅, A3 fine-tuning Gemma langues vernaculaires camerounaises reste à coder bloc H).
- **2026-05-19** — **Session A1 backend — Fondations Identité + Ton + Routing NEXYA (Période 2 IA-Quality Phase A) livrée — 194 nouveaux tests A1 verts en 2.30 s + 0 régression ciblée sur les 95 tests critiques retouchés (test_experts_registry + test_streaming_expert_corpus_concat + test_chat_stream_expert_corpus_injection + test_llm_router + test_chat_stream_persisted).** Mission Silicon Valley : passer NEXYA AI de « IA générique enveloppée d'une marque » à assistant à identité, ton et intelligence de routing irréprochables. **4 nouveaux modules** `app/ai/` (~1100 lignes) : `nexya_tone.py` (10 commandements FR+EN — tutoiement, no creuse opening « Bien sûr ! » bannies, anti-sycophancy, structure scannable, jargon décortiqué, Africa-first contextuel **non-exclusif** validé Ivan « pas mondial, europe surtout, sois sage », brièveté calibrée, exemples concrets, transparence sur limites, chaleur professionnelle mentor grand frère), `nexya_identity.py` (4 paliers progressifs Loth Ivan Ngassa Yimga / dev Flutter / Cameroun / Nexyalabs + sécurité brand technique anti-leak Gemini/GPT/Claude + description vendeuse 11 experts + 15 features magnifiques + anti-hallucination biographique strict + anti-exposition coordonnées perso + résistance prompt injection), `nexya_routing.py` (matrice cross-expert intelligent — cas Ivan : question informatique posée à expert Cuisine → redirect Computer **PAS Général** comme avant ; helper `detect_query_intent` keyword cascade FR+EN safety-critical first ; `get_routing_guidance` instruction LLM avec table d'aiguillage + 5 règles obligatoires ; immune prompt injection car matrice figée Python), `nexya_preamble.py` (assembleur `build_nexya_preamble(expert_id, locale='fr')` avec ordre `tone → routing → identity` figé, cap 12000 chars défaut ~3000 tokens overhead acceptable, fail-safe absolue kill-switch + exception → None pattern aligné D3 memory + G1 corpus, idempotence stricte byte-à-byte). **Wiring `streaming.py`** : 2 modifs chirurgicales — import `build_nexya_preamble` + `_stream_link` ligne 578 nouvel ordre `parts = [nexya_preamble, ctx.memory_context, ctx.expert_corpus_context, rag_block, config.system_prompt]` — preamble EN TÊTE de tout. **Cleanup `experts.py`** : `_NEXYA_IDENTITY` legacy (lignes 97-110) transformé en `""` avec commentaire explicatif (rétrocompat 100 % des 11 concatenations sans toucher aux 11 `_X_PROMPT` qui deviennent purement métier — identité brand désormais injectée en amont). Préserve intégralement `_DOMAIN_GUARDRAIL_TEMPLATE` G2 + disclaimers safety-critical + `corpus_enabled=True` cooking. **3 nouveaux settings** `config.py` + `.env.example` : `nexya_preamble_enabled: bool = True` (kill-switch global), `nexya_preamble_max_chars: int = 12000` (min 500 max 16000), `nexya_preamble_default_locale: Literal['fr','en'] = 'fr'`. **Aucune migration DB, aucune nouvelle dep pip, aucun nouvel endpoint HTTP.** **6 fichiers tests** (194 verts en 2.30 s) : `test_nexya_tone.py` (40 tests — 10 commandements FR+EN paramétrés, formules creuses bannies, Africa-first contextuel testé), `test_nexya_identity.py` (69 tests — 4 paliers FR+EN, Loth Ivan + Cameroun + Flutter présents, hallucinations interdites paramétrées « le plus grand »/« Stanford »/« Harvard »/« diplôme », coordonnées perso jamais exposées, sécurité brand 4 providers à esquiver, 11 experts listés, 15 features marquées), `test_nexya_routing.py` (43 tests — 11 slugs exacts contrat Flutter, 15 happy paths intent detection FR+EN, cas Ivan cooking→computer/cooking→legal/computer→medicine, no-op cases, anti-prompt-injection 4 tentatives), `test_nexya_preamble.py` (35 tests — kill-switch off, cap chars respect, troncature marker FR+EN, fail-safe sur exceptions tone/identity/routing, idempotence byte-à-byte, 11 slugs canoniques), `test_streaming_nexya_preamble_injection.py` (7 tests — preamble en TÊTE du system_prompt, ordre strict preamble→memory→corpus→rag→system, kill-switch comportement pré-A1, builder None → chat continue), + fix collatéral `test_streaming_expert_corpus_concat.py` (fixture autouse `nexya_preamble_enabled=False` pour isoler la concat G1) + `test_chat_stream_expert_corpus_injection.py` (marker "NEXYA" → "Expert Cuisine" post-cleanup). **Décisions architecturales A1→A8** : (A1) injection en TÊTE via `_stream_link` plutôt qu'extension des 11 system_prompts → single point of injection ; (A2) 4 modules dédiés plutôt qu'un méga module → maintenance ciblée ; (A3) multilangue FR+EN dès Session 1 → cible diaspora UE/international ; (A4) pas de « NYLI voice » naming (validation Ivan « oublie ton histoire de nyli voice ») ; (A5) ordre `tone → routing → identity` → routing jamais tronqué (valeur ajoutée critique A1) ; (A6) cap 12000 chars défaut (overhead ~$0.0002/req Flash acceptable) ; (A7) `_NEXYA_IDENTITY=""` plutôt que suppression complète → rétrocompat 100 % ; (A8) fail-safe absolue + kill-switch alignés D3/G1. **Coût A1 : $0** (pure infra Python + tests mock-first, zéro appel LLM). Checklist Ivan post-merge test manuel terrain (5 questions via /chat/stream device réel : « Qui es-tu ? » → palier 1 ; « Qui t'a créé en détail ? » → palier 2/3 sans hallucination ; « Tu utilises Gemini ? » → esquive élégante ; en mode Cuisine « Comment coder une boucle Python ? » → routing vers Informatique **PAS Général** ; « Donne-moi la recette du ndolé » → réponse RAG G2 ton tutoyé scannable). **Boucle Période 2 IA-Quality Phase A ouverte** (A1 ✅ — A2 affûtage prompts experts individuels, A3 fine-tuning Gemma langues vernaculaires camerounaises restent à coder).
- **2026-05-04** — **D3 backend mini-extension `project_id`** (mini-fix non listé dans BACKEND_SESSIONS_PLAN.md, déclenché par la session frontend D3). Extension `ConversationCreate` + `ChatStreamRequest` schemas Pydantic avec `project_id: uuid.UUID | None = None`, `ConversationService.{create, ensure_conversation_for_stream}` étendus avec ownership check via `ProjectService._get_owned_project` (404 IDOR-safe via `ResourceNotFoundException("Projet")`, jamais 403 — anti-énumération UUID), capture `user_id_str = str(user.id)` AVANT `db.commit()` (anti-MissingGreenlet **4ᵉ occurrence** du pattern : ReportService 2026-04-21 + ProjectService.create 2026-05-03 + ProjectService.update 2026-05-03), local import `ProjectService` dans la méthode pour casser le cycle d'import projects↔chat. Ignore silencieux + log debug si `conversation_id != None ET project_id != None` (V1 : pas de mutation cross-feature transparente, le rattachement d'une conv existante sera V2 via `PATCH /chat/conversations/{id}`). Forward `body.project_id` dans `router.chat_stream`. **12 tests verts en isolation** (`tests/features/chat/test_create_with_project_id.py` — 4 sous-classes : Pydantic schemas × 7, service create × 3 happy/IDOR/legacy, service ensure × 2 happy/ignore-silent). **0 migration DB** (FK `conversations.project_id ON DELETE SET NULL` existe depuis migration 006 C2), **0 nouvelle dep pip**, **0 nouvelle exception** (réutilise `ResourceNotFoundException`), **0 nouveau setting pricing TODO Ivan**. Pure plomberie pour débloquer la session frontend D3 (ProjectDetailScreen branché backend conversations en single round-trip via `/chat/stream { project_id }`).
- **2026-05-21** — **F1.5 — Range schedules Planner (full-stack, première feature future promue depuis `project_nexya_future_features.md`) livrée.** Ajoute 3 types de planification au Planner NEXYA, en plus des 6 existants (`once`/`interval_minutes`/`daily`/`weekly`/`monthly`/`yearly`) : `weekly_range` (range continu de jours de semaine, lundi → vendredi), `monthly_range` (range continu de jours du mois, du 15 au 30), `multi_weekday` (jours non-continus, mardi + jeudi). Objectif produit : une tâche « lundi → vendredi » = **1 seule tâche** au lieu de 5 tâches `weekly` séparées (tient dans le quota Free=3). **Backend (5 fichiers)** : `app/features/planner/schemas.py` — 3 classes Pydantic `WeeklyRangeConfig` / `MonthlyRangeConfig` / `MultiWeekdayConfig` + `ScheduleType` 6→9 valeurs + union `ScheduleConfig` discriminée étendue ; `migrations/versions/023_extend_range_schedule_types.py` (NEW) — CHECK `ck_tasks_schedule_type` étendu aux 9 types (`down_revision=022_legacy_updated_at`) ; `app/features/planner/models.py` — `CheckConstraint` ORM corrigé (était figé sur **4 types** `once`/`interval_minutes`/`daily`/`weekly` — oubli F0.5 jamais détecté car les tests planner sont des pure-functions sans INSERT DB ; passé aux 9 types, dette F0.5 résorbée) ; `app/features/planner/scheduler.py` — 3 branches `compute_next_run` + 4 helpers (`_next_in_weekday_set` factorisé, `_make_weekly_range_candidate` gère `start>end` wrap weekend, `_make_monthly_range_candidate`, `_make_multi_weekday_candidate`) ; **bug de la spec mémoire corrigé** : `_make_monthly_range_candidate` clampe `start_day` ET `end_day` au dernier jour réel du mois via `calendar.monthrange()` — sans ce clamp `from_dt.replace(day=30)` crasherait en février. **0 modif** sur `router.py` / `service.py` / `workers/scheduler_tasks.py` (délégation 100 % à `compute_next_run` + `model_dump`). **Frontend (7 fichiers + 1 test)** : enum `TaskFrequency` +3 valeurs + extension `label`/`uiLabel`/`toBackendType`/`fromBackendType`/`scheduleSubtitle` +3 cases + helper `weekdayAbbr` + `TaskModel` +5 champs (`startWeekday`/`endWeekday`/`startDay`/`endDay`/`weekdays`) + constructeur + `copyWith` ; `task_response_to_model_mapper.dart` +5 params +3 cases ; `planner_notifier.dart` propage les 5 champs ; `planner_screen.dart` `_PlannerFormSheet` +5 champs d'état + 3 blocs conditionnels (`_buildWeeklyRangeBlock`/`_buildMonthlyRangeBlock`/`_buildMultiWeekdayBlock`) + pickers `showMenu` + widget `_WeekdayChips` + `_save()` validation 3 cas ; `app_fr.arb` + `app_en.arb` 10 clés. **Validation** : backend planner ciblé **88 tests** (`test_planner_schemas.py` + `test_scheduler_helpers.py`) + **57 tests** régression (service/router/worker/integration) verts, 0 régression ; `flutter analyze lib` → 0 erreur 0 warning ; `flutter test` → **414/414 verts** (16 F1.5 + suite complète) ; `openapi.json` régénéré. **0 nouvelle dépendance pip, 0 nouveau setting, 0 changement de pricing/quota.** Détail complet + checklist Ivan post-merge dans la mémoire `project_nexya_f15_session_state.md`.
- **2026-05-21** — **F1.6 — `yearly_range`, 10ᵉ type de planification (extension de F1.5 sur retour produit Ivan).** F1.5 livrait `monthly_range` (« du 15 au 30 ») mais ce type se répète **tous les mois** sans choix de mois — Ivan a demandé de pouvoir figer un mois précis. F1.6 ajoute le type `yearly_range` : « chaque année, en janvier, du 15 au 30 » — récurrence **annuelle**, mois **figé**, range de jours. **Backend** : classe Pydantic `YearlyRangeConfig` (`month` 1-12 + `start_day`/`end_day` + hour/minute) avec validation `start_day < end_day` strict + les 2 bornes doivent exister dans le mois (30/02 et 31/04 rejetés, 29/02 accepté + clampé à 28 en année non bissextile) ; factorisation `_MAX_DAYS_PER_MONTH` + `_ensure_day_in_month()` partagés avec `YearlyConfig` ; `ScheduleType` 9→10 ; migration `024` (CHECK `ck_tasks_schedule_type` 10 types) ; `CheckConstraint` ORM 9→10 ; `scheduler.py` helper `_make_yearly_range_candidate` (mois figé, année courante puis suivante, clamp `calendar.monthrange()`). 0 modif router/service/worker. **Frontend** : enum `TaskFrequency` 9→10, `TaskModel` +champ `month`, bloc form picker de mois + 2 pickers de jour **bornés au mois choisi** (février 1-29, avril 1-30, clamp auto au changement de mois), clarification du libellé `monthly_range` → « Tous les mois (du X au Y) ». **Validation** : 108 tests planner ciblés (88 F1.5 + 20 F1.6) + 57 régression backend verts ; `flutter analyze` 0/0 ; `flutter test` 420/420 ; `openapi.json` régénéré. 0 nouvelle dépendance, 0 nouveau setting, 0 changement de pricing/quota.
- **2026-05-23** — **Session planner-from-chat hardening — FIX timezone client + LOT C carte vivante + LOT D diagnostic worker (16 nouveaux tests backend verts, 0 régression).** Suite directe planner-from-chat. **FIX timezone** : sans offset client, le LLM interprétait « 20h » comme UTC → tâche programmée 1h trop tard (Cameroun UTC+1). Solution : `ChatStreamRequest.client_timezone: str | None` + parser strict `_parse_client_timezone` dans `build_temporal_context` → injection bloc « horloge utilisateur » avec heure LOCALE + instruction LLM « produis tes ISO avec offset (`2026-05-23T20:00:00+01:00`) ». Côté Flutter : helper `_localIsoOffset()` via `DateTime.now().timeZoneOffset` → `+HH:MM`. **LOT C** : nouveau helper `_build_messages_with_live_task_status` dans `GET /chat/conversations/{id}/messages` (1 SELECT `WHERE id IN (...) AND user_id` IDOR-safe + patch deep-copy de `metadata_json.tool_calls[*].data.task.{status, paused, next_run_at, last_run_at, run_count}` avec valeurs ORM live, `status='deleted'` synthétique pour tâches purgées RGPD ou soft-deleted) → la carte chat affiche le statut RÉEL à chaque réouverture (Programmée → Terminée après exécution). Frontend `TaskPreviewCard` dérive enum `_TaskRunState` (7 états avec précédence stricte) + icône/titre/accent conditionnels + 6 ARB FR+EN. PlannerScreen + NotificationsScreen ajoutent `WidgetsBindingObserver.didChangeAppLifecycleState(resumed)` → refresh auto au retour de background (Africa-first). **LOT D** : nouveau `app/core/observability/worker_health.py` — `check_worker_health(redis_client)` scanne `arq:*` au boot Uvicorn et log warning + CTA inline (`arq workers.worker.WorkerSettings`) si le cron Planner `dispatch_due_tasks_every_1m` est absent. Fail-safe absolu, 0 fail-fast. Justification : bug Ivan 2026-05-22 — vieux worker arq tournait sans le cron, détection impossible sans `redis-cli KEYS arq:*` manuel. **Validation** : 16 nouveaux tests pytest verts (7 LOT C + 9 LOT D) + 57 régression tz + 112 régression chat persisted + 429/429 Flutter + `flutter analyze lib` 0/0. **0 nouvelle migration DB, 0 nouvelle dépendance pip, 0 nouvel endpoint REST, 0 nouveau setting** — pure plomberie diagnostic + enrichissement read-path.
- **2026-05-22** — **Session planner-from-chat — Création d'une tâche planifiée DEPUIS le chat (9 lots, full-stack backend + frontend) livrée.** L'utilisateur tape « rappelle-moi que demain à 8h je dois prendre mes médicaments » dans une conversation et une **carte tâche planifiée** apparaît dans le fil. **Défaut structurel découvert + corrigé** : l'orchestrateur `run_with_tool_rounds` (livré F2, testé F2.5) **n'était JAMAIS appelé en production** — `_run_link` streamait le LLM sans jamais exécuter les tool_calls côté serveur. L'`event: tool_call` partait mais aucune tâche n'était créée. **9 lots** : (1) câblage orchestrateur dans `_run_link` — `execute_tool_call`/`run_with_tool_rounds` prennent `db_session_factory` (la session HTTP est périmée pendant le streaming SSE), anti-double-exécution (round 0 re-raise, round ≥ 1 swallowed), émission `ChatChunk(tool_result=…)` ; (2) nouveau `nexya_temporal.py` — `build_temporal_context()` injecte la date/heure UTC courante EN TÊTE du system prompt (le LLM ne savait pas transformer « demain 8h » en ISO) + `build_tools_guidance()` source unique de la doctrine tools ; (3) `_SCHEDULE_CONFIG_SCHEMA` réécrit 4 → 10 `schedule_type` (le LLM ne pouvait littéralement pas créer un range) ; (4) `medicine`/`legal` `tools_allowed` `False → True` (décision produit Ivan — un patient peut demander un rappel de traitement) ; (5) nouveau `intent_classifier.py` — `detect_planning_intent()` keyword classifier conservateur (se désamorce sur les questions méta) → `tool_config=ANY` forcé via `force_tool_call` quand l'intention est claire (Gemini Flash en `AUTO` ignore trop souvent un `create_task` évident) ; (6) SSE `event: tool_result` (`ToolResultDelta{id, name, success, data, error}`) — le client résout sa carte tâche sur les vraies données backend ; (7) frontend `TaskPreviewCard` schedule humanisé pour les 10 types ; (8) tests — 76 backend (`test_nexya_temporal.py` 18 + `test_intent_classifier.py` ~58 + `test_tools_orchestrator.py` +4) + 9 frontend (`sse_tool_result_test.dart`) ; (9) docs. **0 migration DB, 0 nouvelle dép pip, 0 nouvel endpoint, 0 nouveau code d'erreur** — la table `scheduled_tasks` F1 + les 4 tools F2 + l'orchestrateur F2 existaient déjà, planner-from-chat les a juste enfin câblés. **Validation** : 76 nouveaux tests backend verts ; régression ciblée 301 passed / 2 xfailed ; suite backend complète **2803 passed / 2 failed** — **0 régression planner-from-chat**, les 2 échecs (`test_ci_yaml_structure.py`) sont un drift CI pré-existant hors scope (workflow `deploy.yml` WIP L2 d'Ivan non whitelisté) ; `flutter test` 429/429 + `flutter analyze lib` 0/0. **Coût : $0** (pure infra + tests mock-first).

- **2026-05-26** — **Two-Tier Smart Preamble — refonte ~6000-tokens preamble en CORE/EXTENDED avec safety dédié (3 commits A+B+C, 238+ nouveaux tests verts + 404 régression OK, -33% coût IA moyen, 100% info marketing préservée).** Refonte architecturale du préambule NEXYA injecté dans le system prompt LLM en deux tiers intelligents : **CORE** (toujours injecté ~3900 tokens — tone + identity_core + routing_rules + safety) et **EXTENDED** (injecté UNIQUEMENT sur marketing intent ~2100 tokens additionnels — identity_extended 15 features + routing_table complète). Déclencheur déterministe `_detect_marketing_intent(user_message, locale)` scanne ~30 keywords FR + ~25 EN. 0 appel LLM, 0 latence ajoutée. **Commit A (d3ac1e8)** : identité fondateur 4 paliers progressifs + bio 3-axes (Python+R+Power BI / Flutter / Adobe XD) + règle 5 cohérence session + règle 6 anti-superlatif procédure 3-temps. **Commit B (59bc0e8)** : pipeline Two-Tier complet — accessors `get_identity_core()` / `get_identity_extended()` / `get_capability_teaser()` / `get_routing_table_extended()`, `build_nexya_preamble(*, user_message=None)` qui appelle `_detect_marketing_intent` et injecte EXTENDED conditionnellement. **Commit C (1f2488c)** : nuances tone #2 (autorisation explicite invitations action « Comment puis-je t'aider aujourd'hui ? » + distinction réagit/ouvre) + #7 (profondeur calibrée 2-4 phrases simple / 5-8 complexe) ; nouveau **module `nexya_safety.py`** dédié (pattern miroir tone/identity/routing) — 4 catégories refus FR+EN parité stricte (malware/phishing, désinformation/deepfakes, hate speech, NSFW) + format refus standard avec reformulation positive + résistance prompt injection + anti-moralisation excessive ; compression routing CORE (table markdown supprimée, ~600 chars économisés sur 95% requêtes) ; cap chars bumped 12000→25000 (mesure réelle CORE FR=15692 + CORE+EXTENDED FR=23994) ; tone budget 3500→4500. **Validation chiffrée** : 238/238 tests Commit C verts en 3.42s + 404 régression ciblée + 38 prod-safety, 0 régression sur la suite cible. **Coût IA estimé** : avant ~6000 tokens/req, après 0.95×3923 + 0.05×5999 ≈ 4027 tokens moyen = **-33% économie** (proche target -40% Ivan). **0 nouvelle migration DB, 0 nouvelle dépendance pip, 0 nouvel endpoint HTTP** — pure infra preamble Python.
