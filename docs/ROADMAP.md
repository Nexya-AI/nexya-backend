...
## Backend (`nexya_backend/`) — ~38 % couvert
...
### Tâches
- [ ] **Backend** Modèles `Conversation`, `Message`
- [ ] **Backend** CRUD `/conversations`, `/conversations/{id}/messages`
- [ ] **Backend** `/chat/stream` accepte `conversation_id` optionnel
- [ ] **Backend** Génération automatique titre après 3ᵉ message (résumé LLM, async arq)
- [ ] **Backend** Table `abuse_reports` (user_id, message_id, reason, status)
- [ ] **Backend** `POST /reports`
- [ ] **Backend** Workflow modération (`pending|reviewed|actioned`)
